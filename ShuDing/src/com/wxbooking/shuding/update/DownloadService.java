...
import com.wxbooking.shuding.common.AppConfig;
...
//	private String mTitle ="正在下载%s";
		...
		public void handleMessage(Message msg) {
			...
			case 1:
				...
				if (rate < 100) {
					...
					mNotification.setLatestEventInfo(mContext, getResources().getString(R.string.download_over),
							getResources().getString(R.string.file_download_over), contentIntent);
	...
	public IBinder onBind(Intent intent) {
		downloadUrl = intent.getStringExtra(BUNDLE_KEY_DOWNLOAD_URL).toString().trim();
		saveFileName = saveFileName + getSaveFileName(downloadUrl).toString().trim(); 
	...
	public void onCreate() {
		super.onCreate();
		mTitle = this.getResources().getString(R.string.download_title);
	...
	private void setUpNotification() {
		...
		CharSequence tickerText = getResources().getString(R.string.ready_download);
		...
		RemoteViews contentView = new RemoteViews(getPackageName(),
				R.layout.notification_download_show);
	...
	private void installApk() {
		...
		AndroidUtils.installAPK(mContext, apkfile);
	}
