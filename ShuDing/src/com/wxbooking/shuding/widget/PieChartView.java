...
public class PieChartView extends View {
	...
	private int[] mPieColors = new int[] { Color.RED, Color.GREEN, Color.BLUE,
	...
	private void init(Context context) {
		// init screen
		screenW = AndroidUtils.getScreenWidth(context);
		screenH = AndroidUtils.getScreenHeight(context);

		pieCenterX = screenW / 2;
		// pieCenterY = screenH / 3;
		pieCenterY = (int) (screenW / 2.5);
		pieRadius = screenW / 4;
		smallMargin = AndroidUtils.dip2px(context, 5);
		...
		textPaint.setTextSize(AndroidUtils.dip2px(context, 16));
		...
		linePaint.setStrokeWidth(AndroidUtils.dip2px(context, 1));
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mPieItems != null && mPieItems.length > 0) {
			...
			for (int i = 0; i < mPieItems.length; i++) {
				...
				String itemPercentText = StringUtils.formatFloat(mPieItems[i]
						.getItemValue() / totalValue * 100)
						+ "%";
