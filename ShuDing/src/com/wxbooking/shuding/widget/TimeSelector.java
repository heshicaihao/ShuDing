...
import com.wxbooking.shuding.utils.DateFormatUtils;
...
public class TimeSelector {
	...
	public enum SCROLLTYPE {
		YEAR(1), MONTH(2), DAY(4), HOUR(8), MINUTE(16);
	...
	private int startYear, startMonth, startDay, startHour, startMininute,
			endYear, endMonth, endDay, endHour, endMininute, minute_workStart,
			minute_workEnd, hour_workStart, hour_workEnd;
	...
	public TimeSelector(Context context, ResultHandler resultHandler,
		...
		startCalendar.setTime(DateFormatUtils.parse(startDate, FORMAT_STR));
		endCalendar.setTime(DateFormatUtils.parse(endDate, FORMAT_STR));
		initDialog();
		initView();
	}
	 ...
	 * @param endDate
	 * @param workStartTime
	 *            工作日起始时间 如：朝九晚五 format："09:00"
	 * @param workEndTime
	 *            工作日结束时间 format："17:00"
	 */
	public TimeSelector(Context context, ResultHandler resultHandler,
			String startDate, String endDate, String workStartTime,
			String workEndTime) {
	...
	public void show() {
		if (startCalendar.getTime().getTime() >= endCalendar.getTime()
				.getTime()) {
	...
	private void initView() {
		...
		tv_select.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View view) {
				handler.handle(DateFormatUtils.format(selectedCalender.getTime(),
						FORMAT_STR));
				seletorDialog.dismiss();
			}
		});
	...
	private boolean excuteWorkTime() {
		...
		if (!StringUtils.isEmpty(workStart_str) && !StringUtils.isEmpty(workEnd_str)) {
			...
			startWorkTime.set(Calendar.HOUR_OF_DAY,
					workStartCalendar.get(Calendar.HOUR_OF_DAY));
			startWorkTime.set(Calendar.MINUTE,
					workStartCalendar.get(Calendar.MINUTE));
			endWorkTime.set(Calendar.HOUR_OF_DAY,
					workEndCalendar.get(Calendar.HOUR_OF_DAY));
			endWorkTime.set(Calendar.MINUTE,
					workEndCalendar.get(Calendar.MINUTE));

			if (startTime.getTime().getTime() == endTime.getTime().getTime()
					|| (startWorkTime.getTime().getTime() < startTime.getTime()
							.getTime() && endWorkTime.getTime().getTime() < startTime
							.getTime().getTime())) {
				Toast.makeText(context, "时间参数错误", Toast.LENGTH_LONG).show();
				return false;
			}
			startCalendar
					.setTime(startCalendar.getTime().getTime() < workStartCalendar
							.getTime().getTime() ? workStartCalendar.getTime()
							: startCalendar.getTime());
			endCalendar
					.setTime(endCalendar.getTime().getTime() > workEndCalendar
							.getTime().getTime() ? workEndCalendar.getTime()
							: endCalendar.getTime());
	...
	private void monthChange() {
		...
		selectedCalender
				.set(Calendar.MONTH, Integer.parseInt(month.get(0)) - 1);
		month_pv.setData(month);
		month_pv.setSelected(0);
